...
public class SeekPeaceCommand implements Command, Serializable {
    ...
    public SeekPeaceCommand(Player player) {
        this.player = player;
    }

    public void execute(Game game) throws ChessException {
        // move.doMove(game);
        int result = JOptionPane.showConfirmDialog(null, player.getName()
                + " is seeking for peace, agree?", "Seeking Peace", JOptionPane.YES_NO_OPTION);
